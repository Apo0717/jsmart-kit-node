...
class KitSet {
    ...
    //         let sql = "UPDATE `kit_items` SET ? WHERE `id`=?"; //？1要修改的部分 ?2 在哪個id
    ...
    static async getRows(params={}){
        ...
        let t_sql = `SELECT COUNT(1) num FROM \`kit_items\` ${where}`; //樣板字串，和資料表``衝到，要跳脫
        ...
        if(total){
            ...
            let r_sql = `SELECT * FROM \`kit_items\` ${where} ${orderStr} LIMIT ${(page-1)*perPage}, ${perPage}`;
